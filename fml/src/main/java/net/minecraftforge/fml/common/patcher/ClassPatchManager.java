...
public class ClassPatchManager {
    ...
    public void setup(Side side)
    {
        ...
        try
        {
            ...
            {
               // if (!((Boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment"))) // Unscrew: Remove this line, because there is no such thing as an Unscrew dev workspace.
                {
                    FMLLog.log.fatal("The binary patch set is missing, things are not going to work!");
                }
                return;
            }
            LZMACompressorInputStream binpatchesDecompressed = new LZMACompressorInputStream(binpatchesCompressed); // Unscrew: LzmaInputStream -> LZMACompressorInputStream
